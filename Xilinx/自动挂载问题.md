## 自动挂载问题
xilinx平台自动挂载后无法自动卸载，后续再插入不会自动挂载没反应。
修改petalinux中如下的文件
components/yocto/layers/poky/meta/recipes-core/udev/udev-extraconf/mount.sh
```
#!/bin/sh
#
# Called from udev
...
# ===== FIXED: remove/change unmount logic =====
#
# Bug 1: [ ! -e "/sys/$DEVPATH" ] — kernel sends uevent BEFORE removing
#   sysfs, so on slow embedded systems /sys/$DEVPATH still exists when udev
#   runs this script, skipping the cleanup entirely.
#
# Bug 2: Parsing /proc/mounts for the mount point fails when the disk label
#   contains spaces — kernel escapes space as \040 in /proc/mounts, but
#   umount needs the real path with literal spaces.
#
# Fix: For "remove", skip sysfs check. Use shell glob (not find|grep) to
#   locate cache files. Read the exact mount point name from cache and
#   umount directly — avoids /proc/mounts \040 encoding issue entirely.
if [ "$ACTION" = "remove" ] && [ -x "$UMOUNT" ] && [ -n "$DEVNAME" ]; then
    name="`basename "$DEVNAME"`"

    for tmpfile in /tmp/.automount-*"${name}"; do
        [ -f "$tmpfile" ] || continue
        logger "mount.sh/remove" "cleaning up $DEVNAME, was mounted by the auto-mounter"

        # Read mount point name from cache (set by automount/get_label_name)
        mntname=`cat "$tmpfile"`
        mntpoint="$MOUNT_BASE/$mntname"

        # Unmount using cached name — avoids /proc/mounts \040 encoding bug
        $UMOUNT "$mntpoint" 2>/dev/null

        # Clean up mount directory and cache file
        rm_dir "$mntpoint"
        rm "$tmpfile"
    done
fi
...
if [ "$ACTION" = "change" ] && [ -x "$UMOUNT" ] && [ -n "$DEVNAME" ]; then
    ...
    for tmpfile in /tmp/.automount-*"${name}"; do
        ...
        if [ ! -e "/sys/$DEVPATH" ]; then
            logger "mount.sh/remove" "cleaning up $DEVNAME (media change), was mounted by the auto-mounter"
